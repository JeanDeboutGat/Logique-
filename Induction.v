...
(*
    destruct:  génère un sous-objectif pour chaque constructeur de type inductif.
    induction: génère un sous-objectif pour chaque constructeur de type inductif et 
              fournit une hypothèse d'induction pour les constructeurs définis de manière récursive.
*)
...
(** **** Exercise: 2 stars, advanced, especially useful (plus_comm_informal) 
    ...
    - Premièrement, on suppose que [n = 0]. On doit montre que

        0 + m =  m + 0

        Le  [m] n'est pas connu donc on fait induction sur  [m] 

          - premierèment, on suppose [m = 0], on doit montre que
            0 + 0 = 0 + 0
            qui verifie la reflexité. 

          - suivant on suppose que  [m = S m'], on doit montre que
            0 + S m'  = S m' + 0 

            Par la  definition de [+] on sait que [0 + n = n] et on a déja prouvé que 
            [n + 0 = 0] par [théorème plus_n_O ], donc montre que [0 + S m'  = S m' + 0] 
            équivaut à montre que  [S m' = S m'], ce qui verifie la reflexivité.


     - suivant , on suppose [n = s n'].  On doit montre que 

        s n' + m =  m + s n'

        Le [m] n'est pas connu donc on fait induction sur [m] 

          - premieremment, on suppose que [m = 0], on doit montre que
            s n' + 0 = 0 + s n'

             Par la  definition de [+] on sait que [0 + n = n] et on a déja prouvé que 
             [n + 0 = 0] par [théorème plus_n_O ], donc montre que [s n' + 0 = 0 + s n'] 
             équivaut à montre que  [S n' = S n'], ce qui verifie la reflexivité.

          - suivant on suppose que  [m = S m'], on doit montre que
            S n' + S m'  = S m' + S n',

            Par la  definition de [+] on sait que [(S m') + n =  S (m' + n) = S (n + m')] et
            par l'hypothese de l'induction [IHn' : n' + S n = S n + n' ]. donc montre que
            [n + (S m') = (S m') + n]  equivaut à montre  [ S (n + m) = n + (S m)]. Il nous reste 
            à montre que [ S (n + m) = n + (S m)] ce qui est prouvé par la theorème [plus_n_Sm].
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal) 
    ...
    - Premièrement on montre que  [true = 0 =? 0]. ceci revient à verifié que [0]?=[0]
      ce que est  directement verifié  par la definition [?=].
    
   - Suivant on montre que [true = S n' = S n' ] avec l'hypothèse [true = n' = n'].
     Ce qui est directement vérifie par cet hypothèse d'induction et la definition de la fonction [?=].
...
Theorem plus_ble_compat_l : forall n m p : nat,
  n <=? m = true -> (p ) <=? (p + m) = true.
Proof.
   intros n m p. induction n as [| n']. intros H0m. 
   - simpl. Abort.
...
Theorem bin_to_nat_pres_incr : forall m : bin, 
  bin_to_nat (incr m) = 1 + bin_to_nat m.
Proof.
  intros m.
  induction m as [| m' | m'].
  - simpl. reflexivity. 
...
(* Oui, les nombres ne seront pas égale car pendant la preuve si on essaye d'inclure une assert ou juste une 
  théoreme à part qui verifie "nat_to_bin (incr m) = 1 + nat_to_bin m.",le nat_to_bin (incr m)
  ne generera pas le nombre attendu avec la fonction d'incrementation*)
...
(*nat_to_bin et nat_bin_nat sont déja defini en haut*)

Fixpoint bin_bin (m:bin) : bin :=
  match m with 
  | Z => Z
  | _  => B0 m
  end.
...
Fixpoint normalize (m':bin) : bin :=
  match m' with
  | Z      => Z
  | B0   m' => bin_bin (normalize m')
  | B1 m' => B1 (normalize m')
  end.

Lemma bin_bin_incr: forall m', bin_bin (incr m') = incr (incr (bin_bin m')).
Proof.
  destruct m'.
  - reflexivity.
  - reflexivity.
...
Theorem bin_nat_bin : forall m', nat_to_bin (bin_to_nat m') = normalize m'.
Proof.
  induction m' as [|m'|m'].
  - reflexivity.
  - simpl.
